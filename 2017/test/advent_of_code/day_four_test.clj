(ns advent-of-code.day-four-test
  (:require [advent-of-code.day-four :as df]
            [clojure.test :refer [deftest is]]))

(def testinput "pphsv ojtou brvhsj cer ntfhlra udeh ccgtyzc zoyzmh jum lugbnk
...
(def testinput-part-two "pphsv ojtou brvhsj cer ntfhlra udeh ccgtyzc zoyzmh jum lugbnk
vxjnf fzqitnj uyfck blnl impo kxoow nngd worcm bdesehw
caibh nfuk kfnu llfdbz uxjty yxjut jcea
qiho qif eupwww avyglnj nxzotsu hio lws
xjty usocjsh pivk qnknunc yjcgh bwya djw zpyr
...
(deftest part-one-1
  (is (df/passphrase? "aa bb cc dd ee"))
  (is (not (df/passphrase? "aa bb cc dd aa")))
  (is (df/passphrase? "aa bb cc dd aaa")))

(deftest part-one-2
  (is (= 466 (df/count-true df/passphrase? testinput))))

(deftest part-two-1
  (is (df/no-permutation? "abcde fghij"))
  (is (not (df/no-permutation? "abcde xyz ecdab")))
  (is (df/no-permutation? "a ab abc abd abf abj"))
  (is (df/no-permutation? "iiii oiii ooii oooi oooo"))
  (is (not (df/no-permutation? "oiii ioii iioi iiio"))))

(deftest part-two-2
  (is (= 251 (df/count-true df/no-permutation? testinput-part-two))))
